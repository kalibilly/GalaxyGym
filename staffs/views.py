...
from attendance.models import AttendanceLog, BiometricDevice
...
class StaffUpdateView(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        # Update all active biometric devices linked to eBioServer
        devices = BiometricDevice.objects.filter(is_active=True)
        for device in devices:
            service = BiometricSyncService(device)
            try:
...
class StaffDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["attendance_history"] = self.object.attendance_logs.order_by(
            "-check_in_time"
        )[:8]
        return context
